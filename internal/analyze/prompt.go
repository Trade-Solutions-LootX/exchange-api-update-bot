...
- УЛУЧШЕНИЯ API, которые терминалу стоит внедрить, даже если старое продолжает работать: расширенная глубина стакана (больше уровней, новые каналы стакана), более частые/быстрые потоки (меньший интервал обновления, low-latency эндпоинты, новые WS-домены), новые типы ордеров или параметры (TP/SL в одном запросе, batch-ордера, post-only, iceberg), новые каналы приватных данных, повышение лимитов запросов, новые версии API с большей пропускной способностью. Для таких — importance high (заметный выигрыш для трейдера) или medium (нишевое), needs_terminal_change=true.

НЕ важно (needs_terminal_change=false, importance none/low): листинг/делистинг/переименование ОТДЕЛЬНЫХ монет и пар (терминал берёт список инструментов с биржи динамически), корректировка плеча/tick size/лимитов позиции по конкретным символам, промо/акции/earn/launchpool/airdrop/конкурсы, маркетинг, новости компании, изменения только в веб-интерфейсе или мобильном приложении, фиат/депозиты/выводы, обслуживание кошельков и сетей — если это не меняет контракт торгового API. Делистинг важен только когда закрывается целый тип рынка или API-эндпоинт.
